...
class TestProductRoutes(TestCase):
    ...
    def test_list_by_name(self):
        ...
        response = self.client.get(
            BASE_URL, 
            query_string=f"name={target_name}"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_list_by_category(self):
        ...
        count = len([p for p in products if p.category == target_category])
        
        # Search by category
        response = self.client.get(
            BASE_URL, 
            query_string=f"category={target_category.name}"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_list_by_availability(self):
        ...
        # Get availability to search for
        target_available = products[0].available
        count = len([p for p in products if p.available == target_available])
        
        # Search by availability
        response = self.client.get(
            BASE_URL, 
            query_string=f"available={str(target_available).lower()}"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
